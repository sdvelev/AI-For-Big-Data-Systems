...
pygame.display.set_caption("Problem Solving by State Space Search: A* (Manhattan distance)")
...
def reconstruct_path(parent_of, current, draw, start_time, max_frontier_size):
    ...
    pygame.display.set_caption(f'Time of A* Algorithm (Manhattan distance): {format(working_time, ".2f")}s | '
    ...
    while is_running:
        ...
        for e in pygame.event.get():
            ...
            if e.type == pygame.KEYDOWN and not algorithm_started:
                ...
                counter_start = timer()
                pygame.display.set_caption("Searching for a Solution with A* (Manhattan distance) ...")
                for row in grid:
                    for current_node in row:
